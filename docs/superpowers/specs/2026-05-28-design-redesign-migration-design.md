...
2. **Artboard-compliance gate #2 (post-implementation fidelity audit), before `review-code`.** For surfaces with artboards: compare as-built to the artboards **value-by-value** and **visually at both breakpoints** (desktop 1440 + mobile ~430px) on seeded data via **chrome-devtools-mcp** (NOT the Chrome extension — see the mobile-viewport memory), authenticated. Produce `docs/superpowers/plans/redesign-chunk-NN-<surface>-artboard-audit.md` with a **per-element disposition log** (per finding: **close** vs **keep-as-built** + rationale; kept deviations are recorded once and not re-litigated). Close the material/minor gaps in this chunk so the fixes land in the diff `review-code` sees. Template/precedent: `docs/design/weekly-eats-redesign/AS-BUILT-VS-ARTBOARDS-AUDIT.md`.

   **⚠️ How to actually do the scrub — this is rigorous work, not a screenshot glance.** Chunk 5 shipped a gate-#2 that was a screenshot glance and it whiffed badly: a pantry dialog rendering at **128px** border-radius, the whole index (title/subline/headers/row-actions) wrong, a blue MUI `Alert` in a dark theme, dialogs centered on mobile where the artboard is full-frame — all passed the glance, all caught later by the user. A screenshot tells you it "looks dark and roughly right"; it does **not** tell you a radius is 8× too big or a field is the wrong height. Follow this method:
   1. **Build the surface inventory = your checklist.** List every `Desktop*` / `Mobile*` function in the surface's `docs/design/weekly-eats-redesign/project/artboards-<surface>.jsx`. **Each function is a required surface AND a required sub-state** — edit-mode, empty state, conflict/error state, pending vs accepted, connected vs offline. Default render ≠ all states. Drive each one live; for any you can't reach with the seed (e.g. a unit-conflict needs contrived data, a non-dismissable flow would mutate seed) **verify by source and say so explicitly** in the log — never silently skip.
   2. **Measure, don't eyeball.** For each element, use `evaluate_script` → `getComputedStyle` to read the actual `borderRadius`, `width/height`, `padding`, `gap`, `fontSize`, `fontWeight`, `letterSpacing`, `color`/`backgroundColor`, `border`, and compare each to the exact value in the artboard `.jsx` + the plan §3 spec. This is the only thing that catches sub-pixel/8×/off-by-a-token drift — it's how the 128px radius surfaced. A bare "looks right" is **not** a verdict.
   3. **Walk every element of every surface**, in order: header → eyebrows/labels → each field → each row → banners/pills → footer → buttons. Don't audit the gestalt; audit the parts.
   4. **Run the MUI-gotcha checklist on every component** (these bit chunk 5):
      - **Bare numeric `borderRadius` in `sx` is multiplied by `theme.shape.borderRadius` (=8).** `borderRadius: tokens.radius.xxxl` (16) renders **128px**. Pixel radii MUST be the `` `${tokens.radius.x}px` `` string form. `grep -n "borderRadius: tokens.radius\.[a-z]*[,}]"` (no `px`) across the chunk's components and fix every hit. (`pill`=999 bare renders fully-rounded regardless, harmless but fix for correctness.)
      - **Mobile presentation mode** of every Dialog must match the artboard AND be consistent across the chunk's dialogs: full-frame (`margin:0`, `width/height:100%` on `xs`) where the artboard is full-frame; bottom-sheet (`Drawer`, radius-18 top, grab handle) only where the artboard explicitly shows a sheet; centered-on-mobile is almost always **wrong**.
      - **Nested modals stack wrong on mobile.** A picker/sub-dialog rendered _inside_ a parent `Dialog` can render its bottom-drawer **behind** the still-centered parent (two modals visible). Make sub-dialogs **siblings** and hide the parent while the child is open (`open={open && !childOpen}`).
      - MUI floating-label outlined fields vs the artboard's flat eyebrow-label fields: note as a deviation when reusing shared inputs (`QuantityInput`, `UnitSelector`, etc.) rather than silently passing.
   5. **Drive every interactive state**, don't just screenshot the default: toggle KEEP/SKIP, select/deselect rows, pending vs accepted, the presence pill's connected/offline/failed, empty vs populated lists.
   6. **Data-gap honesty.** When a column/field has no data without an API change (e.g. last-shop dates, shared-member names), decide explicitly — **wire** (read-only API addition, if in scope), **hide**, or **placeholder** — and surface the call to the user. Never ship a column blank/broken and call it "kept as built."
   7. **The log is per element**, each with measured-vs-spec and a disposition. "Surface looks good" is not a log entry.
