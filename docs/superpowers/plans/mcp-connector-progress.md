...
- **2026-05-30 — Phase 2 open questions resolved + §6.4 carryover closed.**
  ...
  (e) **I6 client reaping DEFERRED (final-review note):** `mcpClients.lastUsedAt` + its index ship, and `touchClient()` exists, but no caller updates `lastUsedAt` on use and no reaper job runs yet — `touchClient` is scaffolding for a future reaper. Abandoned DCR registrations therefore persist (safe: they can't authenticate without an approved user). Follow-up: wire `touchClient` into `/token`+`/authorize` and add a TTL/cron reaper.
  (f) **Set `MCP_ISSUER_URL` in production (deploy note):** unset, issuer/resource derive per-request from `x-forwarded-host`. If the connector is reached via a non-canonical host (apex vs www, custom domain vs `.vercel.app`), `verifyToken`'s resource/audience check fails silently ("connected but nothing works"). Pin `MCP_ISSUER_URL` to the canonical connector origin on Production.
