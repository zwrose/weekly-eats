...
| 3   | `recipe-import` skill (skills-over-MCP delivery)             | **in-progress** — built + `npm run check` green + `/review-code` clean (0 findings) + pushed; manual-test plan posted; awaiting live manual verify | [`plan`](2026-05-31-mcp-phase-3-recipe-import-plan.md)                                                                   | [#140](https://github.com/zwrose/weekly-eats/pull/140#issuecomment-4582026777) | —          |
...
> - [x] **✅ DONE (2026-05-31) — Vercel Firewall `mcp-bypass` rule published.** Vercel's automatic **System Mitigations** (NB: not Attack Mode, which is OFF) were challenging non-browser clients on `/api/mcp` + `/.well-known/oauth-*` (429 `x-vercel-mitigated: challenge`), which would have blocked Claude's connector. A custom **Bypass** rule (paths `pre /api/mcp` OR `pre /.well-known/oauth-`) is now live. It's **project-level**, so it already covers production — nothing to redo at launch. Verified: discovery returns 200/401 live. Self-auth (Bearer/PKCE/rate-limit) still applies; browser legs were never affected. Reversible: `vercel firewall rules remove mcp-bypass` + `publish`.
...
- **2026-05-31 — Phase 3 delivers the `recipe-import` skill via skills-over-MCP (supersedes spec §6.6 out-of-band install).** The connector serves its own skills: `/api/mcp` exposes `skills_list` (metadata) + `skills_get` (full `SKILL.md`) with progressive disclosure, advertised through the MCP server `instructions`. Source of truth is a versioned `skills/recipe-import/SKILL.md`; `outputFileTracingIncludes` (`next.config.ts`) bundles `skills/**` into the function so the loader's `fs` reads resolve on Vercel. Path-traversal-safe via an explicit `SKILL_DIRECTORIES` allowlist (`new Set`). Reference-file serving deferred (YAGNI). Researched 2026-05-31: no MCP protocol "skills capability" exists yet — Skills and MCP are complementary standards — so tools-based delivery (cf. `agentskills-mcp`) is the de-facto pattern. The `SKILL.md` prose is validated manually (§8a); the loader + tools are unit-tested.
