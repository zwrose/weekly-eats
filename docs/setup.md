...
```bash
# Full start: run setup-db then start Turbopack dev server
npm run dev

# Fast start: skip setup-db, just start Turbopack dev server
npm run dev:fast
...
All three use [Turbopack](https://turbo.build/pack) via `next dev --turbopack`.
...
Git worktrees allow running multiple development branches simultaneously with fully isolated ports, databases, and `node_modules`.
...
./scripts/worktree-create.sh <branch-name> --no-install

# Create with an empty database (indexes only, no data)
./scripts/worktree-create.sh <branch-name> --empty

# List all worktrees with port and database info
./scripts/worktree-list.sh

# Remove a worktree (interactive menu if no branch specified)
...
| Database | `weekly-eats` (from `.env.local`) | `weekly-eats-<branch>` (cloned from main) |
| `node_modules` | Own copy | Own copy |
| `.env.local` | Manual | Auto-generated from main's `.env.local` |
| `.next/` build cache | Own | Own |

Worktrees are created at `../weekly-eats-worktrees/<branch>/` (sibling directory to the main repo).
