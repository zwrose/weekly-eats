...
## Phase 4: Enhanced Conflict Resolution Dialog and Test Data

- [ ] Task: Seed test meal plan and recipes in the worktree database for manual verification
...
- [ ] Task: Update conflict dialog to show pre-filled values for auto-converted conflicts
...
- [ ] Task: Remove dead code paths and ensure type safety
  - Review `combineExtractedItems` old conflict fallback (`items[0]` take-first) — remove it, replaced by conversion logic
  - Ensure all new types (`isAutoConverted`, `suggestedQuantity`, `suggestedUnit`) are properly typed in `src/lib/meal-plan-to-shopping-list.ts`
  - Verify no unused imports or exports remain

- [ ] Task: Run full validation suite (`npm run check`) and verify coverage
  - All new files in `src/lib/unit-conversion.ts` must have >80% coverage
  - All modified functions in `meal-plan-to-shopping-list.ts` must maintain >80% coverage
