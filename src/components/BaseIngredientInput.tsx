...
export default function BaseIngredientInput({ 
  ...
  const handleAddFoodItem = async (foodItemData: { name: string; singularName: string; pluralName: string; unit: string; isGlobal: boolean; addToPantry?: boolean }) => {
    try {
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              if (!isNaN(value) && value >= 0) {
                ...
                if (ingredient.type === 'foodItem' && ingredient.id && foodItems) {
                  const foodItem = foodItems.find(item => item._id === ingredient.id);
                  if (foodItem) {
