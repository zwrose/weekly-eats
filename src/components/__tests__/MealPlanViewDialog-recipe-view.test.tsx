...
function makeMealPlan(overrides?: Partial<MealPlanWithTemplate>): MealPlanWithTemplate {
  return {
    _id: 'mp-1',
    name: 'Test Meal Plan',
...
function getDefaultProps() {
  return {
    ...
    getMealTypeName: (type: string) => type.charAt(0).toUpperCase() + type.slice(1),
...
describe('MealPlanViewDialog - Recipe View Feature', () => {
  ...
  describe('Clickable recipe items in view mode', () => {
    ...
    it('renders food items as plain text (not clickable)', () => {
      ...
      expect(within(baseElement).queryByRole('link', { name: 'Bread' })).not.toBeInTheDocument();
    });
