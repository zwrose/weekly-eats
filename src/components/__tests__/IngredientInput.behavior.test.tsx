...
describe('IngredientInput - Behavior Tests', () => {
  ...
  describe('Food item creation', () => {
    it('should open create dialog when Enter pressed with no matching options', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('button', { name: /add "new item" as a food item/i })
          ).toBeInTheDocument();
        },
        { timeout: 2000 }
      );
  ...
  describe('Quantity input', () => {
    it('should allow empty quantity input (displays as empty, stores as 0)', async () => {
      ...
      await waitFor(() => {
        expect(onIngredientChange).toHaveBeenCalledWith(expect.objectContaining({ quantity: 0 }));
      });
    ...
    it('should allow positive decimal quantities', async () => {
      ...
      await waitFor(() => {
        expect(onIngredientChange).toHaveBeenCalledWith(expect.objectContaining({ quantity: 1.5 }));
      });
    });
  ...
  describe('Unit selector', () => {
    it('should update unit singular/plural form based on quantity', async () => {
      ...
      await waitFor(
        () => {
          expect(onIngredientChange).toHaveBeenCalledWith(expect.objectContaining({ quantity: 2 }));
        },
        { timeout: 2000 }
      );
    ...
    it('should show singular form when quantity is 1', async () => {
      ...
      await waitFor(
        () => {
          expect(onIngredientChange).toHaveBeenCalledWith(expect.objectContaining({ quantity: 1 }));
        },
        { timeout: 2000 }
      );
