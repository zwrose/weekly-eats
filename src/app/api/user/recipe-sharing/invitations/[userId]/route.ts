...
export async function PUT(
  ...
  try {
    ...
    if (!owner) {
      return NextResponse.json({ error: RECIPE_SHARING_ERRORS.INVITATION_NOT_FOUND }, { status: 404 });
    }
    ...
    await usersCollection.updateOne(
      ...
      {
        $set: {
          'settings.recipeSharing.invitations.$.status': action === 'accept' ? 'accepted' : 'rejected'
        }
      }
    );
