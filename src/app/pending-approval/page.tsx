...
export default function PendingApprovalPage() {
  ...
  if (status === 'unauthenticated' || (status === 'authenticated' && session?.user?.isApproved)) {
    return (
      <AuthenticatedLayout>
        <Container maxWidth="md" sx={{ py: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
        ...
        <Box sx={{ textAlign: 'center', py: 8 }}>
          ...
          <Paper sx={{ p: 4, mt: 4, maxWidth: 600, mx: 'auto' }}>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
              Thank you for signing up! Your account has been created and is now pending approval by
              an administrator.
            </Typography>

            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
              <Button variant="outlined" startIcon={<Logout />} onClick={handleSignOut}>
                Sign Out
              </Button>
            </Box>
          </Paper>
